...
function Header() {
  ...
  const contentUserName = <span><i className="fa fa-user-circle">{separator}<a><Link to="/2-Bank/user">{userName}</Link></a></i>{separator}<i className="fa fa-sign-out">{separator}<span onClick={signOutClick}><a><Link to="/2-Bank/"> Sign Out</Link></a></span></i></span>;
  const contentHome = <a><Link to="/2-Bank/">Home</Link></a>;
